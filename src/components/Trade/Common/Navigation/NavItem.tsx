...
const NavItem = ({ item }: NavItemProps) => {
  ...
  const activeItem = cn(
    'relative pb-2 transtition cursor-pointer border-b-2 min-w-[90px] group hover:border-chilean-fire-600',
  ...
  return (
    <div className={activeItem} onMouseEnter={onHoverEnter} onMouseLeave={onHoverLeave}>
      <Link href={`${item.path}`} className="flex items-center gap-2" onClick={handleRedirect}>
        <div className=" flex items-center justify-center w-9 h-9 p-2 border rounded-lg bg-shark-400 bg-opacity-40 border-shark-400">
      ...
      text-chilean-fire-600 text-xs flex items-center 
        justify-center px-4 py-3 absolute top-0 
        min-w-[90px]
        w-full">
          Coming soon
        </div>
