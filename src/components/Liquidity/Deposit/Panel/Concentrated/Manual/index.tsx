...
const ConcentratedDepositLiquidityManual = ({
  ...
  const [isInverse, setIsInverse] = useState(parseInt(firstToken.address as string) > parseInt(secondToken.address as string))
  ...
  useEffect(() => {
    const asyncFn = async () => {
      ...
      if(currentPercentage == -1) {
        ...
        const priceAndTick1: {price: number, tick: number} = await getPriceAndTick(Number(poolState.price)/1e18*(1-(currentPercentage/100)))
        const priceAndTick2: {price: number, tick: number} = await getPriceAndTick(Number(poolState.price)/1e18*(1+(currentPercentage/100)))
        ...
        setShownPercentage([((1 - (priceAndTick1.price/poolState.price))*100).toFixed(1), (((priceAndTick2.price/poolState.price) - 1)*100).toFixed(1)])
      }
    }
  ...
  const handleCLAdd = async () => {
    ...
    writeContractAsync({ 
      ...
      args: [[
        ...
        lowerTick, 
        higherTick,
        ethers.parseUnits(_firstValue.toString(), 'ether'), 
        ethers.parseUnits(_secondValue.toString(), 'ether'), 
        0, 
        0, 
  ...
  const handleOnTokenValueChange = async (input: any, token: IToken) => {
    ...
    if(firstToken.address === token.address) {
      setFirstValue(parseFloat(input) != 0 ? (Number(input) % 1 === 0 ? input : parseFloat(input).toFixed(10)).replace(/(\.\d*?[1-9])0+$|\.$/, '$1') : input)
      setSecondValue(parseFloat((input * ratio).toString()).toFixed(10).replace(/(\.\d*?[1-9])0+$|\.$/, '$1'))
    } else {
      setSecondValue(parseFloat(input) != 0 ? (Number(input) % 1 === 0 ? input : parseFloat(input).toFixed(10)).replace(/(\.\d*?[1-9])0+$|\.$/, '$1') : input)
      setFirstValue(parseFloat((input / ratio).toString()).toFixed(10).replace(/(\.\d*?[1-9])0+$|\.$/, '$1'))
    }
  }
