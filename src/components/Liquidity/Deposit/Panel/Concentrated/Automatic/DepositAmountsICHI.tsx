...
const DepositAmountsICHI = ({
  ...
  useEffect(() => {
    tokenList && setToken0InfoData(tokenList.find((t) => t?.address?.toLowerCase() === token0.toLowerCase()) || null)
  }, [token0, tokenList])
  const [isToken0ApprovalRequired, setIsToken0ApprovalRequired] = useState(false)

  const vaultAddress =
    allIchiVaultsByTokenPair?.find((vault) => {
      if (vault.tokenA.toLowerCase() === token0.toLowerCase() && vault.allowTokenA) {
        return true
      }
      if (vault.tokenB.toLowerCase() === token0.toLowerCase() && vault.allowTokenB) {
        return true
      }
      return false
    }) || null
      ...
      <div className="bg-shark-400 bg-opacity-40 px-[15px] py-[29px] md:px-[19px] border border-shark-950 rounded-[10px] mb-2.5">
        <div className="flex w-full xl:w-3/5 justify-between mb-2">
          ...
          <span className="text-xs leading-normal text-shark-100 mr-4 flex items-center gap-x-2">
            ...
            {tokenList?.find((t) => t?.address?.toLowerCase() === token0.toLowerCase())?.symbol}
          </span>
        </div>
                  ...
                  >
                    <div className="flex justify-center items-center gap-3">
                      {selected !== 'Choose one' ? (
                        <>
                          <Image
                            src={`/static/images/tokens/${tokenList?.find((t) => t?.address?.toLowerCase() === token0.toLowerCase())?.symbol}.svg`}
                            alt="token"
