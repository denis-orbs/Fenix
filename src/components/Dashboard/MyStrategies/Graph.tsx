...
const Graph = ({ tickLower, tickUpper, token0Symbol, token1Symbol }: GraphProps) => {
      ...
      <div className="flex items-start relative h-20">
        ...
        {showtoken0 && (
          <>
            ...
            <p className="bg-shark-300 bg-opacity-40 border border-shark-400 text-white text-xs px-3 py-1 rounded-sm mx-auto mt-1">
              Max. Price: {Number(tickUpper.price0) > 1e12 ? '∞' : Number(tickUpper.price0).toFixed(5)} {token0Symbol}/
              {token1Symbol}
            </p>
          </>
        )}
